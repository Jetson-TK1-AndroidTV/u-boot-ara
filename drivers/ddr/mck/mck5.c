...
int dram_init(void)
{
	 ...
	 */
	if (gd->ram_size < CONFIG_SYS_RELOC_END)
		hang();
	else
		gd->ram_size = CONFIG_SYS_RELOC_END;
#endif

	/*
	 * set mck lcd port p6 to high to avoid flicker issue.
	 */

#ifdef CONFIG_PXA1928
	writel(0xfb002000, MCK5_ADDR_BASE + 0x84);
	writel(0x00018000,  CIU_MCK5_PRI);
#endif
